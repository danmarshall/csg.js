...
class CAG {
    ...
    union(cag) {
        ...
        r = r.union(cags.map(function (cag) {
    ...
    extrude(options) {
        ...
        for (let i = 0; i < twiststeps; i++) {
            ...
            let c2 = new Connector(offsetVector.times((i + 1) / twiststeps), [0, 0, offsetVector.z], normalVector.rotateZ((i + 1) * twistangle / twiststeps));
    ...
    rotateExtrude(options) {
        ...
        if (alpha > 0 && alpha < 360) {
            ...
            polygons = polygons.concat(this._toPlanePolygons({ toConnector: connS, flipped: true }));
            polygons = polygons.concat(this._toPlanePolygons({ toConnector: connE }));
        }
        let connT1 = connS;
        let connT2;
