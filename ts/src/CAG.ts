import { EPS, angleEPS, areaEPS, defaultResolution3D } from './constants'
...
import { parseOptionAs3DVector, parseOptionAsBool, parseOptionAsFloat, parseOptionAsInt } from './optionParsers'
import FuzzyCAGFactory from './FuzzyFactory2d'
import CSG from './CSG'
import * as primitives2d from './primitives2d'
import * as CAGMakers from './CAGFactories' 
...
class CAG {
  ...
  static Vertex = Vertex2D;
  static Side = Side;

  static circle = primitives2d.circle;
  static ellipse = primitives2d.ellipse;
  static rectangle = primitives2d.rectangle;
  static roundedRectangle = primitives2d.roundedRectangle;
  static fromCompactBinary = primitives2d.fromCompactBinary;

  static fromPointsNoCheck = CAGMakers.fromPointsNoCheck;
  static fromObject = CAGMakers.fromObject;
   ...
   * @returns {CAG} new CAG object
   */
  static fromPoints = function (points: (Vector2D | number[])[]) {
  ...
  extrudeInOrthonormalBasis(orthonormalbasis: OrthoNormalBasis, depth: number, options?) {
  ...
  // options:
  //   {symmetrical: true}  // extrude symmetrically in two directions about the plane
  extrudeInPlane(axis1: string, axis2: string, depth: number, options) {
  ...
  toCompactBinary() {
    ...
    cag.sides.map(function (side) {
      [side.vertex0, side.vertex1].map(function (v: Vertex2D) {
