import { fnNumberSort } from './utils';
import FuzzyCSGFactory from './FuzzyFactory3d';
import Tree from './trees';
import { EPS } from './constants';
import { reTesselateCoplanarPolygons } from './math/polygonUtils';
...
//let {fromPolygons} from './CSGMakers') // FIXME: circular dependency !
/** Class CSG
...
class CSG {
    ...
    toCompactBinary() {
        let csg = this.canonicalized(), numpolygons = csg.polygons.length, numpolygonvertices = 0, numvertices = 0, vertexmap = {}, vertices = [], numplanes = 0, planemap = {}, polygonindex = 0, planes = [], shareds = [], sharedmap = {}, numshared = 0;
        ...
        let numVerticesPerPolygon = new Uint32Array(numpolygons), polygonSharedIndexes = new Uint32Array(numpolygons), polygonVertices = new Uint32Array(numpolygonvertices), polygonPlaneIndexes = new Uint32Array(numpolygons), vertexData = new Float64Array(numvertices * 3), planeData = new Float64Array(numplanes * 4), polygonVerticesIndex = 0;
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (sidetagtocheck === null)
                        break; // sidestocheck is empty, we're done!
                                    ...
                                    if ((t > 0) && (t < 1)) {
                                        ...
                                        if (distancesquared < (EPS * EPS)) {
                                            ...
                                            if (isNaN(newpolygon.plane.w)) {
                                                let found = false, loop = function (callback) {
