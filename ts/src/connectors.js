...
export class Connector {
    ...
    getTransformationTo(other, mirror, normalrotation) {
        ...
        let axesplane = Plane.anyPlaneFromVector3Ds(new Vector3D(0, 0, 0), us.axisvector, other.axisvector);
...
ConnectorList._fromPath2DExplicit = function (path2D, angleIsh) {
    ...
    let result = new ConnectorList(path2D.points.map(function (p2, i) {
        return new Connector(p2.toVector3D(0), Vector3D.Create(1, 0, 0).rotateZ(getAngle(angleIsh, p2, i)), ConnectorList.defaultNormal);
    }, this));
    result.closed = path2D.closed;
    return result;
};
