...
*/
class Line2D {
  constructor(private normal: Vector2D, public w: number) {
    normal = new Vector2D(normal)
    w = parseFloat(w as any)
    let l = normal.length()
    // normalize:
    w *= l
  ...
  static fromPoints (p1: Vector2D, p2: Vector2D) {
    ...
    absDistanceToPoint (point: Vector2D) {
    ...
    intersectWithLine (line2d: Line2D) {
      let p = solve2Linear(this.normal.x, this.normal.y, line2d.normal.x, line2d.normal.y, this.w, line2d.w)
      const point = new Vector2D(p) // make  vector2d
      return point
    }
