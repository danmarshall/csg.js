...
import { EPS, angleEPS } from '../constants';
import { parseOptionAs2DVector, parseOptionAsFloat, parseOptionAsInt, parseOptionAsBool } from '../optionParsers';
import { defaultResolution2D } from '../constants';
...
class Path2D {
  ...
  constructor(points: (Vector2D | number[])[], private closed?: boolean) {
    closed = !!closed
    points = points || []
    ...
    points.map(function (point_: Vector2D | number[]) {
      var point = new Vector2D(point_)
      let skip = false
      if (prevpoint !== null) {
  ...
  */
  ...
  appendBezier(controlpoints: Vector2D[], options) {
   ...
   */
  appendArc(endpoint: Vector2D | number[], options) {
    let decimals = 100000
    if (arguments.length < 2) {
      options = {}
    }
