import CSG from './CSG';
import { parseOption, parseOptionAs3DVector, parseOptionAs2DVector, parseOptionAs3DVectorList, parseOptionAsFloat, parseOptionAsInt } from './optionParsers';
import { defaultResolution3D, defaultResolution2D, EPS } from './constants';
...
const cube = function (options) {
    ...
    let result = CSG.fromPolygons([
        ...
        let vertices = info[0].map(function (i) {
            let pos = new Vector3D(c.x + r.x * (2 * +!!(i & 1) - 1), c.y + r.y * (2 * +!!(i & 2) - 1), c.z + r.z * (2 * +!!(i & 4) - 1));
